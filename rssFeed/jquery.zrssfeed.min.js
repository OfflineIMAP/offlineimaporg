...
html+='<div class="rssHeader">'+'<a href="'+qualifyLink(feeds.link,options.linkbase)+'" title="'+feeds.description+'">'+feeds.title+'</a>'+'</div>';html+='<div class="rssBody">'+'<ul>';for(var i=0;i<feeds.entries.length;i++){var entry=feeds.entries[i];var pubDate;if(entry.publishedDate){var entryDate=new Date(entry.publishedDate);var pubDate=entryDate.toLocaleDateString()+' '+entryDate.toLocaleTimeString();}
html+='<li class="rssRow '+row+'">'+'<'+options.titletag+'><a href="'+qualifyLink(entry.link,options.linkbase)+'" title="View this feed at '+feeds.title+'" target="'+options.linktarget+'">'+entry.title+'</a></'+options.titletag+'>'
...
for(var m=0;m<xmlMedia.length;m++){var xmlUrl=xmlMedia[m].getAttribute("url");var xmlType=xmlMedia[m].getAttribute("type");var xmlSize=xmlMedia[m].getAttribute("length");html+='<li><a href="'+qualifyLink(xmlUrl,options.linkbase)+'" title="Download this media">'+xmlUrl.split('/').pop()+'</a> ('+xmlType+', '+formatFilesize(xmlSize)+')</li>';}
html+='</ul></div>'}
html+='</li>';}
...
function qualifyLink(link, base){if(link.indexOf("://")!=-1){return link;};if(link.length == 0){return base;};if(link.charAt(0)=='/'){sep=""}else{sep="/";};return base + sep + link;}
